...
int main()
{
    ...
    while (true)
    {
        ...
        if (question_num != 0) npixel = decidePixel();
        if(question_num == 1 || question_num == 2 || question_num == 3 || question_num == 4)
            doInterpolation(mi, question_num, npixel);
        if (question_num == 0)
        {
            cout << "End Homewrok 4 ... " << endl << endl;
            break;
        }
        else
        {
